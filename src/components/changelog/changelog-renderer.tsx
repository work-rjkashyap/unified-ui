...
function parseChangelog(raw: string): ChangelogEntry[] {
  ...
  /** Reference to the items array that last received a top-level `- ` item */
  let lastItemList: string[] | null = null;
  ...
  for (const line of lines) {
    ...
    // Match section heading: ### 🚀 Features, ### Fixed, etc.
    ...
    // Match top-level list items: - description
    const itemMatch = line.match(/^- (.+)/);
    if (itemMatch) {
      const text = itemMatch[1].trim();
      const target = currentSubsection
    ...
    // Continuation / indented sub-item lines — append to the last item
    // Matches: "  - sub item", "  plain continuation text", etc.
    if (lastItemList && lastItemList.length > 0) {
      const trimmed = line.trim();
      if (trimmed.length === 0) continue; // skip blank lines

      // Indented sub-item "  - something" → append with newline
      const subItemMatch = line.match(/^\s+- (.+)/);
      if (subItemMatch) {
        lastItemList[lastItemList.length - 1] +=
          `\n  - ${subItemMatch[1].trim()}`;
        continue;
      }

      // Plain continuation text (indented but no bullet)
      if (/^\s+/.test(line)) {
        lastItemList[lastItemList.length - 1] += ` ${trimmed}`;
      }
    }
  }
...
type SectionColorSet = { bg: string; text: string; dot: string };

const sectionColors: Record<string, SectionColorSet> = {
  // Keep-a-Changelog names
  Added: {
  ...
  // Conventional-commit / emoji-prefixed names
  Features: {
    bg: "bg-green-500/10 dark:bg-green-500/15",
    text: "text-green-700 dark:text-green-400",
    dot: "bg-green-500",
...
const defaultColor: SectionColorSet = {
  bg: "bg-fd-muted/50",
  text: "text-fd-muted-foreground",
  dot: "bg-fd-muted-foreground",
};

/** Strip leading emoji from a section title, e.g. "🚀 Features" → "Features" */
...
function getSectionColor(title: string) {
  return (
    sectionColors[title] ?? sectionColors[stripEmoji(title)] ?? defaultColor
  );
}
...
function ChangelogItem({ text }: { text: string }) {
  // Items may contain embedded newlines from multi-line commit messages
  // e.g. "Main description\n  - Sub detail 1\n  - Sub detail 2 (commit_link)"
  const lines = text.split("\n");
  const mainLine = lines[0];
  const subLines = lines.slice(1).filter((l) => l.trim().length > 0);

  return (
    <li className="relative pl-5 text-sm leading-7 text-fd-muted-foreground before:absolute before:left-0 before:top-3.25 before:h-1.5 before:w-1.5 before:rounded-full before:bg-fd-border">
      <span>{renderInlineMarkdown(mainLine)}</span>
      {subLines.length > 0 && (
        <ul className="mt-1 space-y-0.5 text-fd-muted-foreground/80">
          {subLines.map((sub, i) => {
            // Strip leading "- " from sub-items
            const cleaned = sub.replace(/^\s*-\s*/, "").trim();
            return (
              <li
                key={i}
                className="relative pl-4 text-[13px] leading-6 before:absolute before:left-0 before:top-2.75 before:h-1 before:w-1 before:rounded-full before:bg-fd-border/60"
              >
...
/** Check whether a section title normalises to one of the given keywords */
function sectionIs(title: string, ...keywords: string[]) {
  const plain = stripEmoji(title).toLowerCase();
  return keywords.some((k) => plain === k.toLowerCase());
}
...
function ChangelogStats({ entries }: { entries: ChangelogEntry[] }) {
  ...
  const totalFeatures = entries.reduce((acc, e) => {
    for (const s of e.sections) {
      if (sectionIs(s.title, "Added", "Features", "Planned")) {
        acc += s.items.length;
  ...
  const totalFixes = entries.reduce((acc, e) => {
    for (const s of e.sections) {
      if (sectionIs(s.title, "Fixed", "Bug Fixes")) {
        acc += s.items.length;
        for (const sub of s.subsections) {
          acc += sub.items.length;
        }
...
export function ChangelogRenderer() {
  ...
  if (entries.length === 0) {
    return (
      <div className="rounded-xl border border-fd-border bg-fd-card/40 p-8 text-center">
        <p className="text-fd-muted-foreground">No changelog entries found.</p>
      </div>
    );
  }
