...
export function LayoutBody({
	...
	return (
		<div
			...
			className={cn(
				"grid overflow-x-clip min-h-(--fd-docs-height)  transition-[grid-template-columns] auto-cols-auto auto-rows-auto [--fd-docs-height:94dvh] [--fd-header-height:4rem] [--fd-toc-popover-height:0px] [--fd-sidebar-width:0px] [--fd-toc-width:0px]",
				className,
			)}
			style={
				{
...
export function NavbarLinkItem({
	...
	if (item.type === "menu") {
		return (
			<NavbarLinkItemMenu item={item} className={className} {...props} />
		);
	}
	return (
		<LinkItem
...
function NavbarLinkItemMenu({
	...
	return (
		...
		>
			...
			>
				{item.url ? (
					<LinkItem item={item as { url: string }}>
						{item.text}
					</LinkItem>
				) : (
					item.text
