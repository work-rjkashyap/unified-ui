...
const itemVariants = cva(
	"relative flex flex-row items-center gap-2.5 rounded-md px-2 py-2 text-start text-fd-muted-foreground transition-all duration-300 hover:bg-fd-accent/40 hover:text-fd-foreground data-[active=true]:bg-fd-primary/10 data-[active=true]:text-fd-primary data-[active=true]:font-semibold data-[active=true]:ring-1 data-[active=true]:ring-fd-primary/20 [&_svg]:size-4 [&_svg]:shrink-0 [&_svg]:transition-transform hover:[&_svg]:scale-110 mb-0.5 last:mb-0",
	{
		variants: {
			variant: {
				...
				>
					...
					<aside
						...
						className={cn(
							...
							ref.current &&
								(ref.current.getAttribute("data-collapsed") ===
									"true") !==
									collapsed &&
...
export function SidebarDrawer({
	...
	return (
		<>
			<Base.SidebarDrawerOverlay className="fixed z-60 inset-0 backdrop-blur-sm bg-fd-background/50 data-[state=open]:animate-fd-fade-in data-[state=closed]:animate-fd-fade-out" />
			<Base.SidebarDrawerContent
				className={cn(
					"fixed text-sm flex flex-col shadow-2xl border-s border-fd-border/40 end-0 inset-y-0 w-[85%] min-w-[320px] z-60 bg-fd-background/60 backdrop-blur-xl data-[state=open]:animate-fd-sidebar-in data-[state=closed]:animate-fd-sidebar-out overflow-hidden",
					className,
				)}
				{...props}
			>
...
export function SidebarSeparator({
	...
	return (
		<Base.SidebarSeparator
			className={cn(
				"mt-6 mb-2 px-2 text-[10px] font-bold uppercase tracking-[0.2em] text-fd-muted-foreground/60 flex items-center gap-2 before:content-[''] before:h-px before:flex-1 before:bg-fd-border/30 after:content-[''] after:h-px after:flex-1 after:bg-fd-border/30",
				className,
			)}
			style={{
