{{--
    Unified UI — Form Group Component
    https://unified-ui.space

    A form field group wrapper that handles label, help text, and error
    message layout. Provides consistent vertical spacing and structure
    for form controls, integrating with Laravel's validation error bag.

    Usage:
        {{-- Basic form group --}}
        <x-ui-form-group label="Username" name="username">
            <x-ui-input name="username" />
        </x-ui-form-group>

        {{-- With help text --}}
        <x-ui-form-group label="Username" name="username" help="Choose a unique username.">
            <x-ui-input name="username" />
        </x-ui-form-group>

        {{-- With explicit error --}}
        <x-ui-form-group label="Email" name="email" error="Please enter a valid email address.">
            <x-ui-input name="email" type="email" />
        </x-ui-form-group>

        {{-- Auto-detecting Laravel validation errors --}}
        <x-ui-form-group label="Password" name="password">
            <x-ui-input name="password" type="password" />
        </x-ui-form-group>

        {{-- Required field --}}
        <x-ui-form-group label="Full Name" name="name" required>
            <x-ui-input name="name" />
        </x-ui-form-group>

        {{-- Optional indicator --}}
        <x-ui-form-group label="Bio" name="bio" optional>
            <x-ui-textarea name="bio" />
        </x-ui-form-group>

        {{-- Without label (label is sr-only) --}}
        <x-ui-form-group name="search" sr-only-label label="Search">
            <x-ui-input name="search" placeholder="Search…" />
        </x-ui-form-group>

        {{-- With description below label --}}
        <x-ui-form-group
            label="Notifications"
            name="notifications"
            description="Choose which notifications you'd like to receive."
        >
            <x-ui-checkbox name="notifications_email" label="Email" />
            <x-ui-checkbox name="notifications_sms" label="SMS" />
        </x-ui-form-group>

        {{-- Horizontal layout --}}
        <x-ui-form-group label="Country" name="country" layout="horizontal">
            <x-ui-select name="country" placeholder="Select a country">
        ...
        {{-- Different sizes --}}
        <x-ui-form-group label="Name" name="name" size="sm">
            <x-ui-input name="name" size="sm" />
        </x-ui-form-group>

        {{-- Custom error bag --}}
        <x-ui-form-group label="Email" name="email" bag="login">
            <x-ui-input name="email" type="email" />
        </x-ui-form-group>

        {{-- With inline hint (beside label) --}}
        <x-ui-form-group label="API Key" name="api_key" inline-hint="Read-only">
            <x-ui-input name="api_key" readonly />
        </x-ui-form-group>

    Props:
        name         — the form field name, used for auto-detecting validation errors and label association
        label        — optional label text rendered above the control
        description  — optional description text rendered below the label
        help         — optional help text rendered below the control
        error        — optional explicit error message (overrides auto-detected validation errors)
        bag          — Laravel error bag name to check for validation errors (default: "default")
        required     — boolean, shows a red asterisk on the label (default: false)
        optional     — boolean, shows "(optional)" text on the label (default: false)
        disabled     — boolean, applies disabled styling to the entire group (default: false)
        size         — sm|md|lg (default: md)
        layout       — vertical|horizontal (default: vertical)
        labelWidth   — Tailwind width class for the label column in horizontal layout (default: "w-1/3")
        srOnlyLabel  — boolean, visually hides the label but keeps it accessible (default: false)
        inlineHint   — optional hint text displayed inline next to the label
        id           — optional explicit id for the label's "for" attribute (defaults to name)
--}}
