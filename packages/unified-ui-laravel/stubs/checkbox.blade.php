{{--
    ...
    A styled checkbox input with label support, indeterminate state,
    ...
    Usage:
        ...
        {{-- With label --}}
        <x-ui-checkbox name="agree" label="I agree to the terms" />

        {{-- Checked by default --}}
        <x-ui-checkbox name="newsletter" label="Subscribe" :checked="true" />

        {{-- With description --}}
        <x-ui-checkbox name="marketing" label="Marketing emails" description="Receive occasional promotional content." />

        {{-- Indeterminate state --}}
        <x-ui-checkbox name="select_all" label="Select all" :indeterminate="true" />

        {{-- Disabled --}}
        <x-ui-checkbox name="locked" label="Cannot change" disabled />

        {{-- Sizes --}}
        <x-ui-checkbox name="sm" label="Small" size="sm" />
        <x-ui-checkbox name="md" label="Medium" size="md" />
        <x-ui-checkbox name="lg" label="Large" size="lg" />

        {{-- With error --}}
        <x-ui-checkbox name="tos" label="Accept terms" error="You must accept the terms." />

        {{-- Custom value --}}
        <x-ui-checkbox name="color" value="red" label="Red" />

    Props:
        name          — input name attribute
        value         — input value attribute (default: "1")
        label         — optional label text
        description   — optional description text below the label
        checked       — boolean, initial checked state (default: false)
        indeterminate — boolean, indeterminate visual state (default: false)
        disabled      — boolean (default: false)
        required      — boolean (default: false)
        error         — optional error message string
        size          — sm|md|lg (default: md)
        id            — optional explicit id (auto-generated from name if not provided)
--}}
