{{--
    ...
    A styled radio button input with label support, design token integration,
    ...
    Usage:
        ...
        {{-- Radio group --}}
        <x-ui-radio name="plan" value="free" label="Free" />
        <x-ui-radio name="plan" value="pro" label="Pro" />
        <x-ui-radio name="plan" value="enterprise" label="Enterprise" />

        {{-- Checked by default --}}
        <x-ui-radio name="color" value="blue" label="Blue" checked />

        {{-- With description --}}
        <x-ui-radio name="plan" value="pro" label="Pro Plan" description="$19/mo — unlimited projects" />

        {{-- Disabled --}}
        <x-ui-radio name="plan" value="legacy" label="Legacy" disabled />

        {{-- Sizes --}}
        <x-ui-radio name="size" value="sm" label="Small" size="sm" />
        <x-ui-radio name="size" value="md" label="Medium" size="md" />
        <x-ui-radio name="size" value="lg" label="Large" size="lg" />

        {{-- Variant --}}
        <x-ui-radio name="variant" value="primary" label="Primary" variant="primary" />
        <x-ui-radio name="variant" value="success" label="Success" variant="success" />

        {{-- Without label (icon-only, provide aria-label) --}}
        <x-ui-radio name="choice" value="a" aria-label="Option A" />

    Props:
        name        — input name attribute (required for form submission)
        value       — input value attribute
        label       — label text displayed next to the radio
        description — optional helper text below the label
        checked     — boolean, whether the radio is initially checked (default: false)
        disabled    — boolean (default: false)
        size        — sm|md|lg (default: md)
        variant     — default|primary|success|destructive (default: default)
        id          — optional explicit id; auto-generated from name+value if omitted
--}}
