{{--
    ...
    A multi-line text input with design token styling, auto-resize support,
    character counting, and accessible labeling.

    Usage:
        {{-- Basic textarea --}}
        <x-ui-textarea name="message" placeholder="Type your message…" />

        {{-- With rows --}}
        <x-ui-textarea name="bio" :rows="5" placeholder="Tell us about yourself" />

        {{-- Auto-resizing --}}
        <x-ui-textarea name="notes" auto-resize placeholder="Auto-growing textarea" />

        {{-- With character limit --}}
        <x-ui-textarea name="tweet" :maxlength="280" show-count />

        {{-- Disabled / readonly --}}
        <x-ui-textarea name="locked" disabled>Read-only content</x-ui-textarea>
        <x-ui-textarea name="readonly" readonly value="Cannot edit" />

        {{-- Variants --}}
        <x-ui-textarea name="msg" variant="error" />
        <x-ui-textarea name="msg" variant="success" />

        {{-- Sizes --}}
        <x-ui-textarea name="msg" size="sm" />
        <x-ui-textarea name="msg" size="lg" />

    Props:
        variant    — default|error|success (default: default)
        size       — sm|md|lg (default: md)
        rows       — number of visible text rows (default: 3)
        maxlength  — maximum character length (default: null)
        showCount  — boolean, show character count below the textarea (default: false)
        autoResize — boolean, auto-grow height based on content (default: false)
        disabled   — boolean (default: false)
        readonly   — boolean (default: false)
        name       — form field name
        value      — initial value (can also use default slot)
