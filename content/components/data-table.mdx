...
When working with controlled state, import the relevant state types:
...
<div className="flex flex-col gap-6 w-full max-w-3xl mt-4">
	<div>
		<span className="block text-xs font-medium text-ds-muted-foreground mb-2">
			compact
		</span>
...
| Prop        | Effect                                                            |
| ----------- | ----------------------------------------------------------------- |
| `striped`   | Alternating row background using `--ds-color-muted` on even rows  |
| `bordered`  | Visible borders between all cells using `--ds-color-border-muted` |
...
<DataTable
	...
	emptyState={
		<div className="flex flex-col items-center gap-3 py-8">
			<SearchX className="size-8 text-ds-muted-foreground" />
...
The caption is rendered visually below the table body, styled with `text-ds-muted-foreground`. It also accepts `ReactNode`:
...
<DataTable
	...
	footer={(table) => (
		<p className="text-xs text-ds-muted-foreground px-2">
...
- **Focus rings** — All interactive elements (checkboxes, pagination buttons, filter inputs, column toggle, sort headers) display visible focus rings using `--ds-color-focus-ring`.
...
| Token                           | Usage                                               |
| ------------------------------- | --------------------------------------------------- |
| `--ds-color-primary`            | Checkbox accent color, focus ring base              |
| `--ds-color-border`             | Table borders, input borders, pagination buttons    |
| `--ds-color-border-muted`       | Row dividers, column filter input borders           |
| `--ds-color-muted`              | Striped row background, header background, skeleton |
| `--ds-color-foreground`         | Cell text, button text, header text                 |
...
| `--ds-color-popover-foreground` | Column visibility dropdown text                     |
| `--ds-color-primary-muted`      | Selected row background highlight                   |
| `--ds-color-focus-ring`         | Focus ring on all interactive elements              |
| `--ds-radius-sm`                | Filter inputs, checkboxes, skeleton bars            |
| `--ds-radius-md`                | Search input, pagination buttons, dropdown          |
| `--ds-shadow-md`                | Column visibility dropdown shadow                   |
| `--ds-duration-fast`            | Hover and focus transition speed                    |
| `--ds-z-dropdown`               | Column visibility dropdown z-index (default: 40)    |
...
[data-ds-component="data-table"] {
	--ds-color-primary: 37 99 235;
	--ds-color-primary-muted: 219 234 254;
	--ds-color-focus-ring: 59 130 246;
}
```
