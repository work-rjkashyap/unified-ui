...
All animation timing originates from the [motion tokens](/docs/design-system/tokens#motion). The preset layer builds on these tokens — you never hardcode duration or easing values.
...
```tsx
import { duration, easing, spring, durationSeconds } from "@work-rjkashyap/unified-ui/tokens";
...
```tsx
import { motion, AnimatePresence } from "framer-motion";
import { overlayBackdrop, modalContent, modalContentSpring } from "@work-rjkashyap/unified-ui";
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          <motion.div
            {...overlayBackdrop}
            className="fixed inset-0 bg-black/50 z-ds-overlay"
          />
          {/* Content */}
          <motion.div
            {...modalContentSpring}
            className="fixed inset-0 flex items-center justify-center z-ds-modal"
          >
            <div className="bg-ds-background rounded-ds-lg shadow-ds-lg p-6">
              {children}
            </div>
          </motion.div>
        </>
...
<motion.div
  ...
  className="bg-ds-surface-raised rounded-ds-md p-4 border border-ds-border cursor-pointer"
>
  Interactive card
</motion.div>
...
<motion.button
  ...
  className="bg-ds-primary text-ds-primary-foreground px-4 py-2 rounded-ds-md"
>
  Click me
</motion.button>
```
...
// Skeleton pulse
<motion.div {...pulse} className="h-4 w-32 bg-ds-muted rounded-ds-sm" />

// Loading spinner
<motion.div {...spin} className="size-5 border-2 border-ds-primary border-t-transparent rounded-full" />
```
...
function AnimatedCard({ children }) {
  ...
  return (
    <div
      className={prefersReduced ? "" : "transition-transform hover:-translate-y-1"}
    >
      {children}
    </div>
  );
...
>
  This content will animate in normally, but appear instantly
  when the user prefers reduced motion.
</MotionSafe>
```

<Callout type="info">
  All 24+ built-in components (Dialog, Sheet, Toast, Accordion, Tabs, etc.) already use reduced-motion-aware animation internally. You only need these hooks and utilities when building custom animated components.
</Callout>
...
import { fadeIn, slideUp, scaleIn, motionProps, useReducedMotion } from "@work-rjkashyap/unified-ui";
...
// Raw motion tokens
import { duration, easing, spring, stagger } from "@work-rjkashyap/unified-ui/tokens";
```
...
<Callout type="warn">
  **Performance tip:** Framer Motion adds ~30KB to your bundle. If you only need CSS transitions (hover states, color changes), use Tailwind's `transition-*` utilities instead of Framer Motion. Reserve Framer Motion for enter/exit animations, springs, and layout animations.
</Callout>
