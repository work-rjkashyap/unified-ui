...
```
TypeScript constants → CSS custom properties (--ds-*) → Tailwind utilities (ds-*)
```
...
| -------------- | ------------------ | ----------------------- | --------------- | -------------------------------- |
| **Colors**     | `tokens/colors.ts` | `--ds-color-*`          | `bg-ds-*`       | `bg-ds-primary`                  |
| **Spacing**    | `tokens/spacing.ts`| `--ds-spacing-*`        | `p-ds-*`        | `p-ds-4` (16px)                  |
| **Typography** | `tokens/typography.ts` | `--ds-font-*`       | `font-ds-*`     | `font-ds-sans`                   |
| **Radius**     | `tokens/radius.ts` | `--ds-radius-*`         | `rounded-ds-*`  | `rounded-ds-md` (6px)            |
| **Shadows**    | `tokens/shadows.ts`| `--ds-shadow-*`         | `shadow-ds-*`   | `shadow-ds-md`                   |
| **Z-Index**    | `tokens/z-index.ts`| `--ds-z-*`              | `z-ds-*`        | `z-ds-modal` (40)                |
| **Motion**     | `tokens/motion.ts` | `--ds-duration-*`       | —               | `--ds-duration-normal` (200ms)   |
...
// In Tailwind classes — use ds-* utilities
...
<Callout type="warn">
  **Avoid large spacing values for dense UI.** Per project guidelines, utilities like `py-24+`, `space-y-10+`, and `gap-12+` should be used sparingly. Prefer compact, information-dense layouts.
</Callout>
...
<Callout type="warn">
  **Never hardcode z-index values.** Always reference the token scale. The `max` value (9999) is an escape hatch — if you need it, document why.
</Callout>
...
```tsx
import { duration, easing, spring, durationCSS, easingCSS } from "@work-rjkashyap/unified-ui/tokens";
...
import { spacing, radius, fontSize, duration, zIndex } from "@work-rjkashyap/unified-ui/tokens";
...
Use the `ds-*` utility prefix:

```html
<div class="p-ds-4 rounded-ds-md font-ds-sans bg-ds-primary text-ds-primary-foreground" />
```
...
| ------------------------- | --------------------------- | ------------------------------------------- |
| **Semantic over literal** | `--ds-color-primary` not `--ds-color-indigo-600` | Intent survives palette changes |
| **Kebab-case CSS vars**   | `--ds-font-family-sans`     | Standard CSS custom property convention     |
...
<Callout type="info">
  The `--ds-` prefix is a **permanent decision**. It namespaces all design system properties to prevent collisions with host application styles, Fumadocs (`--fd-*`), or any third-party CSS.
</Callout>
