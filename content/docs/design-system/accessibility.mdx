...
The standard focus ring uses the `--ds-color-focus-ring` token (brand.500 in light mode, brand.400 in dark mode) with a 2px ring offset:
...
// Standard button
<button className={`px-4 py-2 rounded-ds-md bg-ds-primary text-ds-primary-foreground ${focusRingClasses}`}>
  Save
</button>

// Compact for inline links
<a href="/docs" className={`text-ds-primary underline ${focusRingCompactClasses}`}>
  Read the docs
</a>
...
<div className={`border border-ds-border rounded-ds-md p-4 ${focusWithinRingClasses}`}>
...
```tsx
import { focusRingClassList, focusRingCompactClassList, focusRingInsetClassList } from "@work-rjkashyap/unified-ui/utils";
...
- The border color changes to `--ds-color-danger`
- The focus ring color changes to `--ds-color-danger`
...
```tsx
import { checkHexContrast, contrastRatio, meetsAA, meetsAAA } from "@work-rjkashyap/unified-ui/utils";
...
```tsx
import { parseHex, relativeLuminance, toRGBString, parseRGBString } from "@work-rjkashyap/unified-ui/utils";
...
See the [Motion](/docs/design-system/motion#reduced-motion) page for the full reduced motion API reference (`useMotion`, `useMotionProps`, `withReducedMotion`, `MotionSafe`, etc.).
...
```tsx
// Playwright
await page.locator('[data-ds-component="button"][data-ds-variant="primary"]').click();
